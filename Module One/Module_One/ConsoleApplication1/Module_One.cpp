...
// Added a "bool& success" parameter so the caller can check if an overflow happened
{
    success = true; // Initialize success to true; set to false only if an overflow is detected
    T result = start;

    for (unsigned long int i = 0; i < steps; ++i)
    {
        //  Check at compile-time if T is a floating-point type (like float, double)
        if constexpr (std::is_floating_point<T>::value)
        {
            T temp = result + increment;
            if (std::isinf(temp)) { // If the result is infinite, overflow occured, but only for floating point types
                success = false; // Inform the caller that an overflow occurred
                return result;   // return results so far
            }
            result = temp;
        }
        else
        {
            // For integers, check if adding increment would exceed max value
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps, bool& success)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // Check if we're working with floating point types
        if constexpr (std::is_floating_point<T>::value)
        {
            T temp = result - decrement;
            if (std::isinf(temp)) {   // If result is infinite, underflow occurred
                success = false;   // Report the underflow
        ...
        {
            // If result is smaller than what we want to subtract, underflow would occur
            if (result < decrement) {
                success = false;
                return result;
            }
...
void test_overflow()
{
    ...
    if (success)
        ...
        std::cout << "Error: An Overflow occured!" << std::endl;
    ...
    if (success)
        ...
        std::cout << "Error: An Overflow occurred!" << std::endl;
}
...
void test_underflow()
{
    ...
    if (success)
        ...
        std::cout << "Error: An Underflow occurred!" << std::endl;
